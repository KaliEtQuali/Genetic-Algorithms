...
class Player(GameObject):
    ...
    def __init__(self, file, sprite_width, sprite_height, position = Point(0,0), Player2=False, alt_color = None):
        file = '../res/Char/'+file+'/'+file
        if alt_color:
            #spritefile = file+'Clones/{}.png'.format(alt_color)
            spritefile = file+'{}.png'.format(alt_color)
...
#        surface.convert_alpha()
    ...
    def mainloop(self, bot1=None, bot2=None):
        ...
        while True:
            ...
            if not end_round and self.color[3] == 0:
                if self.ui.timer.time == 0 or self.character1.health.amIdead() or self.character2.health.amIdead():
                    ...
                    bot1.round_over(info, round_marker)
                    bot2.round_over(info, round_marker)
                else:
    ...
    #                self.character1.print_inputs()
            ...
            info = [
                self.ui.timer.time,
                self.character1.health.health,
                self.character1.energy.energy,
                self.character1.sprite_width,
                self.character1.sprite_height,
                self.character1.getPrintPoint().x,
                self.character1.getPrintPoint().y,
                self.character1.getState(),
                self.character1.facingRight,
                self.character2.health.health,
                self.character2.energy.energy,
                self.character2.sprite_width,
                self.character2.sprite_height,
                self.character2.getPrintPoint().x,
                self.character2.getPrintPoint().y,
                self.character2.getState(),
                self.character2.facingRight
            ]

            if end_round:
                bot1.tap_key(bot1.light)
            else:
                bot1.nn_stuff(info)
                # if training bot vs bot then nn_stuff
                #bot2.nn_stuff(info)
                # if training bot vs random then random_stuff
                bot2.random_stuff(info)
            ## Ticking ##
            #############
    ...
    def getInputs(self):
        ...
        for event in pygame.event.get():
            ...
            if event.type == KEYDOWN:
                ...
                if event.key == self.keysP1[0]:
                    stick_inputs_p1.append(KEYCONST.UP)
                if event.key == self.keysP1[1]:
                ...
                ## Get Player 2 move keys
                if event.key == self.keysP2[0]:
                    stick_inputs_p2.append(KEYCONST.UP)
                if event.key == self.keysP2[1]:
...
if __name__ == "__main__":
    ...
    nn1 = simplest_neural_net_ever()
    bot1 = Internal_bot(1, nn1)
    player2 = Player('Rick', 120, 100, Player2=True)
    nn2 = simplest_neural_net_ever()
    bot2 = Internal_bot(2, nn2)
