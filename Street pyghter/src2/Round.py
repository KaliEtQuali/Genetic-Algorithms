...
class Game:
    ...
    def __init__(self, screen, background, character1, character2):
        ...
        # For bots Use
        self.connexions = []
        self.connexionP1 = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        self.connexionP2 = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        self.connexionP1.bind(('',12891))
        self.connexionP2.bind(('',12892))
        self.connexionP1.listen(5)
        self.connexionP2.listen(5)
        try:
            connexion1, infos_connexion1 = self.connexionP1.accept()
            self.connexions.append(connexion1)
        except socket.timeout:
        	print("To late")
        try:
            connexion2, infos_connexion2 = self.connexionP2.accept()
            self.connexions.append(connexion2)
        except socket.timeout:
        	print("To late")
...
#        surface.convert_alpha()
            ...
            if not end_round and self.color[3] == 0:
                if self.ui.timer.time == 0 or self.character1.health.amIdead() or self.character2.health.amIdead():
                    ...
                    for connexion in self.connexions:
                        connexion.send(pickle.dumps(round_marker))
                    print(round_marker)
                    '''
                else:
    ...
    #                self.character1.print_inputs()
                ...
                if self.endwin:
                    if self.color[3] < 255:
                        ...
                        if self.initRound(round_marker[0], round_marker[1]):
                            print("YOLO")
                            for connexion in self.connexions:
                                connexion.send(pickle.dumps([]))
                                connexion.close()
                            self.connexionP1.close()
                            self.connexionP2.close()
                            break
                else:
            ...
            for connexion in self.connexions:
                connexion.send(pickle.dumps(info))
            ## Ticking ##
            #############
